...
void main()
{
	...
	vec3 initial_sample = (v_position + 1) / 2;
	float v = texture3D(u_texture, initial_sample);
	float step = u_quality;
	vec4 color_acc = vec4(0,0,0,0); 
	vec4 color_i = vec4(v.x, v.x, v.x, v.x); //from texture

	for(int i = 0; i < 4000; i++)
	{
		color_i.rgb = color_i.rgb * color_i.a;
		color_acc = step * color_i * (1.0 - color_acc.a) + color_acc;
		if(color_acc.a >= 1.0) //OR outside volume
			break;	
	}
